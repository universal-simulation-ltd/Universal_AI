...
export default defineConfig(({ mode }) => {
  // ⚠️ `mobile` mode is what the Capacitor (iOS) build uses, and its one job is
  // to leave the SERVICE WORKER out. Capacitor serves the bundle from a
  // `capacitor://localhost` origin; a `sw.js` + `registerSW.js` built for
  // `https://opensource.unisim.co.uk` has no business inside the app, and this
  // build shipped all three (`sw.js`, `registerSW.js`, `workbox-*.js`) into
  // `ios/App/App/public` for its whole life. It never *broke* — WKWebView does
  // not support service workers on a custom scheme and registerSW.js guards on
  // `'serviceWorker' in navigator` — but that is a WebKit implementation detail
  // holding up dead code, not a design. Every other Capacitor app in the suite
  // strips it; now so does this one. Build via `npm run cap:sync`, which also
  // runs `scripts/verify-mobile-bundle.mjs` to prove the copied bundle is clean.
  //
  // `desktop` is accepted as an alias because that is the mode name the sibling
  // apps use for the same build (they have Electron targets; this one does not).
  //
  // NB: unlike the siblings, `base` does NOT change here. They are served under
  // a path prefix (`/blackbook/`, `/polling/`) so their production asset URLs
  // are wrong inside the app and they need a relative base; this app's base is
  // `/` everywhere, and `capacitor://localhost/assets/...` resolves to the
  // copied bundle's own root. Nothing to correct.
  const isNative = mode === 'mobile' || mode === 'desktop'
  return {
    define: {
